...
class Tu01Win(GlutRendererWindow):
    ...
    def initialize(self):
        ...
        shader.init_shader_from_glsl_files("glsl/tu02/vertex.glsl", "glsl/tu02/fragment.glsl")
        # shader var ids
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(model.vertices) * 4, (GLfloat * len(model.vertices))(*model.vertices),
                     GL_STATIC_DRAW)

        self.context.uv_buffer = glGenBuffers(1)
        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, self.context.uv_buffer)
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(model.texcoords) * 4,
                     (GLfloat * len(model.texcoords))(*model.texcoords), GL_STATIC_DRAW)
        ...
        glBufferData(GL_ELEMENT_ARRAY_BUFFER, len(model.indices) * 2, (GLushort * len(model.indices))(*model.indices),
                     GL_STATIC_DRAW)

    def calc_mvp(self, width=1920, height=1080):
        self.context.projection_matrix = glm.perspective(glm.radians(45.0), float(width) / float(height), 0.1, 1000.0)
