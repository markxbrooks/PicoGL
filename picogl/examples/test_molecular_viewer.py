...
def test_pdb_loading():
    ...
    try:
        ...
        for chain_id, count in chain_counts.items():
            color = (
                "Green" if chain_id == "A" else "Blue" if chain_id == "B" else "White"
            )
            ...
            for i, atom in enumerate(calpha_atoms[:5]):
                print(
                    f"   {i+1}. Chain {atom.chain_id}, Residue {atom.res_seq} ({atom.res_name}): "
...
def test_molecular_viewer_import():
    ...
    try:
        ...
        from legacy_qt_molecular_viewer import (LegacyMolecularViewerWindow,
