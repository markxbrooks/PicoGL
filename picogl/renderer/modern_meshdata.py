...
class ModernMeshData:
    ...
    def __init__(self, vertices, normals=None, uvs=None, colors=None, indices=None):
        ...
        GL.glBufferData(GL.GL_ARRAY_BUFFER, vertices.astype(np.float32), GL.GL_STATIC_DRAW)
        ...
        if normals is not None:
            ...
            GL.glBufferData(GL.GL_ARRAY_BUFFER, normals.astype(np.float32), GL.GL_STATIC_DRAW)
        ...
        if colors is not None:
            ...
            GL.glBufferData(GL.GL_ARRAY_BUFFER, colors.astype(np.float32), GL.GL_STATIC_DRAW)
        ...
        if uvs is not None:
            ...
            GL.glBufferData(GL.GL_ARRAY_BUFFER, uvs.astype(np.float32), GL.GL_STATIC_DRAW)
        ...
        if indices is not None:
            ...
            GL.glBufferData(GL.GL_ELEMENT_ARRAY_BUFFER, indices.astype(np.uint32), GL.GL_STATIC_DRAW)
